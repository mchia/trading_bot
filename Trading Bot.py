...
class strategies():
    class RSIStrategy(bt.Strategy):
        ...
        def transaction_data(self):
            ...
            transaction_data['strategy'] = strategies.RSIStrategy.__name__
    ...
    class GoldenCross(bt.Strategy):
        ...
        def notify_trade(self, trade):
            ...
            pnlcomm = thousand_separator(trade.pnlcomm)
            sumcomm = thousand_separator(trade.pnlcomm - trade.pnl)
            self.trade_results.append([self.trade_id, round(trade.pnl, 0), round(trade.pnlcomm, 0), round(sb.cerebro.broker.get_cash(), 0)])
        ...
        def transaction_data(self):
            buy_table = pd.DataFrame(data=self.buy_transactions, columns=['id', 'entry_date', 'entry_price', 'buying_fee', 'shares'])
            sell_table = pd.DataFrame(data=self.sell_transactions, columns=['id', 'exit_date', 'exit_price', 'selling_fee'])
            results_table = pd.DataFrame(data=self.trade_results, columns=['id', 'gross_earnings', 'net_earnings', 'acc_bal'])
            transaction_table = pd.merge(buy_table, sell_table, on='id', how='inner')
            transaction_data = pd.merge(transaction_table, results_table, on='id', how='inner')
            transaction_data['total_fees'] = transaction_data['buying_fee'] + transaction_data['selling_fee']
            ...
            data_to_insert = []
            for index, row in transaction_data.iterrows():
                data = {col: row[col] if col != 'Datetime' else row[col].strftime('%Y-%m-%d %H:%M:%S') for col in transaction_data.columns}
                data_to_insert.append(data)
